...
namespace Ink.Runtime
{
    ...
    public class Profiler
    {
        ...
        /// </summary>
        public ProfileNode rootNode { get { return _rootNode; } }
        ...
        internal void Step(CallStack callstack)
        {
            ...
            _currStepDetails = new StepDetails { type = stepType, obj = currObj };
        ...
        public string StepLengthReport()
        {
            ...
            var averageStepTimes = _stepDetails.GroupBy(s => s.type)
                .Select(typeToDetails => new KeyValuePair<string, double>(typeToDetails.Key, typeToDetails.Average(d => d.time)))
                .OrderByDescending(stepTypeToAverage => stepTypeToAverage.Value).Select(stepTypeToAverage =>
                                                                                        {
            ...
            var accumStepTimes = _stepDetails.GroupBy(s => s.type)
                .Select(typeToDetails =>
                            new KeyValuePair<string, double>(typeToDetails.Key + " (x" + typeToDetails.Count() + ")", typeToDetails.Sum(d => d.time)))
                .OrderByDescending(stepTypeToAccum => stepTypeToAccum.Value).Select(stepTypeToAccum =>
                                                                                    {
...
#pragma warning restore 0649
        ...
        public int totalMillisecs { get { return (int) _totalMillisecs; } }
