...
//		}
        ...
        public static void LoadAndPlay(TextAsset storyJSONTextAsset)
        {
            ...
            if (window.story != null)
            {
                if (EditorUtility.DisplayDialog("Story in progress",
                    "The Ink Player Window is already playing a story. Would you like to stop it and load the new story?", "Stop and load", "Cancel"))
                {
                    window.Stop();
        ...
        void DisplayStoryControlGroup()
        {
            ...
            if (story != null)
            {
                var profileButtonTitle = new GUIContent(isProfiling ? "Stop Profiling" : "Profile", (isProfiling ? "Stop" : "Start") + " Profiling");
            ...
            EditorGUI.BeginChangeCheck();
            playerOptions.continueAutomatically = GUILayout.Toggle(playerOptions.continueAutomatically,
            ...
            playerOptions.chooseAutomatically = GUILayout.Toggle(playerOptions.chooseAutomatically,
        ...
        void DrawStory()
        {
            ...
            EditorGUI.BeginChangeCheck();
            displayOptions.displayTime = GUILayout.Toggle(displayOptions.displayTime,
                new GUIContent("Time", "Displays the datetime when the content was shown"), EditorStyles.toolbarButton);
            if (EditorGUI.EndChangeCheck())
            {
                ScrollToBottom();
            }
            ...
            EditorGUI.BeginChangeCheck();
            displayOptions.displayChoicesInLog = GUILayout.Toggle(displayOptions.displayChoicesInLog, new GUIContent("Choices", "Displays choices"),
            ...
            EditorGUI.BeginChangeCheck();
            displayOptions.displayDebugNotesInLog = GUILayout.Toggle(displayOptions.displayDebugNotesInLog,
                new GUIContent("Debug", "Displays load/save/set variable events"), EditorStyles.toolbarButton);
            if (EditorGUI.EndChangeCheck())
            {
                ScrollToBottom();
            }
        ...
        void DisplayLine(string content)
        {
            ...
            EditorGUILayout.SelectableLabel(content, EditorStyles.wordWrappedLabel, GUILayout.ExpandHeight(true), GUILayout.Width(width),
...
//			}
        ...
        void DrawDivertsPanel()
        {
            ...
            {
                storyHistory.Add(new InkPlayerHistoryContentItem("Diverted to '" + divertPanelState.divertCommand + "'",
                    InkPlayerHistoryContentItem.ContentType.DebugNote));
        ...
        void DrawFunctionsPanel()
        {
            ...
            {
                storyHistory.Add(new InkPlayerHistoryContentItem("Execute function '" + functionPanelState.functionName + "'",
                    InkPlayerHistoryContentItem.ContentType.DebugNote));
            ...
            functionInputList.drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                                                    {
                                                        ...
                                                        input.type = (FunctionPanelState.FunctionInput.FunctionInputType) EditorGUI.EnumPopup(typeRect,
                                                            input.type);
...
//						story.ListItemWithName(input.inkVariablePath).Values
                                                                    if (!StringIsWhiteSpace(input.inkVariablePath))
                                                                        input.inkVariableValue = story.variablesState[input.inkVariablePath];
        ...
        void DrawVariablesPanel()
        {
            ...
            {
                if (!StringIsWhiteSpace(variablesPanelState.searchString) &&
                    !StringContains(variable, variablesPanelState.searchString, StringComparison.OrdinalIgnoreCase)) continue;
                EditorGUILayout.BeginHorizontal();
            ...
            {
                storyHistory.Add(new InkPlayerHistoryContentItem(
                    "Change '" + variableToChange + "' from '" + story.variablesState[variableToChange] + "' to '" + newVariableValue + "'",
                    InkPlayerHistoryContentItem.ContentType.DebugNote));
        ...
        void DrawProfilerData()
        {
            ...
            {
                var path = EditorUtility.SaveFilePanel("Save mega log", "", "megalog.txt", "txt");
    ...
    public class UndoHistory<T> where T : class
    {
        ...
        public bool canUndo { get { return undoHistory.Count > 0 && undoHistoryIndex > 0; } }

        public bool canRedo { get { return undoHistory.Count > 0 && undoHistoryIndex < undoHistory.Count - 1; } }
        ...
        public UndoHistory(int maxHistoryItems)
            : this()
        {
