...
namespace Ink.UnityIntegration
{
    ...
    {
        public static bool compiling { get { return InkLibrary.Instance.compilationStack.Count > 0; } }
        ...
        static void UpdateProgressBar()
        {
            ...
            string message = "Compiling .Ink File " + (InkLibrary.Instance.compilationStack.Count - numCompiling) + " of " +
                             InkLibrary.Instance.compilationStack.Count + ".";
        ...
        {
            Debug.LogError(
                "Entered Play Mode while Ink was still compiling. Story will not be up to date. This should never happen. Recommend exiting and re-entering play mode.");
        }
        ...
        internal static void CompileInkInternal(InkFile inkFile)
        {
            ...
            {
                Debug.LogWarning(
                    "Inklecate (the ink compiler) not found in assets. This will prevent automatic building of JSON TextAsset files from ink story files.");
                return;
            }
            ...
            {
                Debug.LogError("Due to a Unity bug, Inklecate path cannot contain an apostrophe. Ink will not compile until this is resolved. Path is '" +
                               inklecatePath + "'");
                return;
            }
            ...
            string outputPath = InkEditorUtils.CombinePaths(inkFile.absoluteFolderPath, Path.GetFileNameWithoutExtension(Path.GetFileName(inkFile.filePath))) +
                                ".json";
        ...
        static void OnCompileProcessComplete(object sender, EventArgs e)
        {
            Process process = (Process) sender;
        ...
        private static void DelayedComplete()
        {
            ...
            if (longestTimeTaken > InkSettings.Instance.compileTimeout * 0.6f)
                Debug.LogWarning(
                    "Compilation took over 60% of the time required to timeout the compiler. Consider increasing the compile timeout on the InkSettings file.");
        ...
        private static Regex _errorRegex =
            new Regex(@"(?<errorType>ERROR|WARNING|TODO|RUNTIME ERROR):(?:\s(?:'(?<filename>[^']*)'\s)?line (?<lineNo>\d+):)?(?<message>.*)");
